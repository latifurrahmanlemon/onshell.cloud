...
function TrendChart({ points }: { points: AnalyticsData["trend"] }) {
  ...
  return <div className="analytics-chart-wrap">
    <div className="analytics-chart-legend" aria-hidden="true"><span><i className="is-views" />Page views</span><span><i className="is-visitors" />Visitors</span></div>
  ...
  return <div className="analytics-dashboard">
    ...
    {!data && resource.loading ? <div className="analytics-loading" aria-label="Loading analytics"><span /><span /><span /><span /></div> : data && <>
      ...
      <section className="admin-card analytics-live-card">
        ...
        <div className="admin-table-wrap"><table className="admin-table analytics-recent"><thead><tr><th>Visitor</th><th>Page</th><th>Location</th><th>Device</th><th>Active time</th><th>When</th></tr></thead><tbody>{data.recent.rows.map((row) => <tr key={row.id}><th>{row.user ? <span>{row.user.name}<small>{row.user.email}</small></span> : <span>Anonymous<small>visitor {row.visitorId?.slice(0, 8) ?? "unknown"} · session {row.sessionId?.slice(0, 8) ?? "unknown"}</small></span>}</th><td className="analytics-path" title={row.path}>{row.path}</td><td>{[row.city, row.country].filter(Boolean).join(", ") || "Unknown"}</td><td>{[row.browser, row.deviceType].filter(Boolean).join(" · ") || "Unknown"}</td><td>{duration(row.durationMs)}</td><td>{formatDateTime(row.createdAt)}</td></tr>)}</tbody></table></div>

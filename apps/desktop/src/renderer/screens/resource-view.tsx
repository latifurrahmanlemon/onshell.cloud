...
export function HostsView({ hosts, onClose, onOpen, onEdit, onFavorite }: BaseProps & { onOpen(host: Host): void; onFavorite(host: Host): void; onEdit(host: Host | "new"): void }) {
  const [query, setQuery] = useState("");
  const [view, setView] = useState<ViewMode>("grid");
  const [favoritesOnly, setFavoritesOnly] = useState(false);
  const [environment, setEnvironment] = useState("all");
  const [sort, setSort] = useState("name");
  const visible = hosts.filter((host) => (!favoritesOnly || host.isFavorite) && (environment === "all" || host.environment === environment) && `${host.name} ${host.address} ${host.username ?? ""} ${host.environment} ${host.tags.join(" ")}`.toLowerCase().includes(query.trim().toLowerCase())).sort((a, b) => sort === "address" ? a.address.localeCompare(b.address) : Number(Boolean(b.isFavorite)) - Number(Boolean(a.isFavorite)) || a.name.localeCompare(b.name));
  return <section className="resource-view" aria-labelledby="hosts-title">
    ...
    <div className="resource-filters"><label><Icon name="search" size={14}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search hosts"/></label><button className="button" aria-pressed={favoritesOnly} onClick={() => setFavoritesOnly((value) => !value)}>★ Favorites</button><select aria-label="Filter environment" value={environment} onChange={(event) => setEnvironment(event.target.value)}><option value="all">All environments</option>{[...new Set(hosts.map((host) => host.environment))].map((value) => <option key={value} value={value}>{value}</option>)}</select><select aria-label="Sort hosts" value={sort} onChange={(event) => setSort(event.target.value)}><option value="name">Favorites, then name</option><option value="address">IP / address</option></select><ViewToggle value={view} onChange={setView}/><button className="button button--primary" onClick={() => onEdit("new")}><Icon name="plus" size={14}/>Add host</button></div>
    <div className={`host-resource host-resource--${view}`}>
      {visible.map((host) => <article className="host-resource-card" key={host.id}><span className="credential-card__icon"><Icon name={host.isLocal ? "computer" : "host"}/></span><div className="host-resource-card__identity"><h2>{host.name}</h2><p className="selectable">{host.username ? `${host.username}@` : ""}{host.address}{host.port !== 22 ? `:${host.port}` : ""}</p></div><span className="host-resource-card__environment">{host.environment}</span><div className="host-resource-card__actions"><button className="icon icon--framed" aria-pressed={Boolean(host.isFavorite)} aria-label={`${host.isFavorite ? "Unfavorite" : "Favorite"} ${host.name}`} onClick={() => onFavorite(host)}>{host.isFavorite ? "★" : "☆"}</button><button className="icon icon--framed" onClick={() => onOpen(host)} title="Connect" aria-label={`Connect to ${host.name}`}><Icon name="play" size={13}/></button>{!host.isLocal && !host.isAgent && <button className="icon icon--framed" onClick={() => onEdit(host)} title="Edit" aria-label={`Edit ${host.name}`}><Icon name="gear" size={13}/></button>}</div></article>)}
      {visible.length === 0 && (
